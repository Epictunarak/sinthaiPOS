...
## 7. `ActivityLogs` (ระบบสร้างให้เอง)

บันทึกทุกครั้งที่มีคนแก้ไขข้อมูลผ่าน API (ขาย, ยกเลิกบิล, ตรวจนับที่มีส่วนต่าง,
บันทึกบาร์โค้ด) ไว้ตรวจย้อนหลังได้ว่าใครทำอะไรเมื่อไหร่ ต่อท้ายอย่างเดียว ไม่ลบ

| คอลัมน์ | ชนิด | คำอธิบาย |
|---|---|---|
| Timestamp | datetime | เวลาที่ทำรายการ |
| UserId | text | ใครเป็นคนทำ |
| Action | text | sale / void / stocktake / setBarcode |
| Details | text | สรุปสั้นๆ ว่าทำอะไร (เช่น SKU และค่าก่อน-หลัง) |
