...
## เมนูสำหรับเจ้าของร้าน (ไม่ต้องแตะโค้ด)

หลังอัปโค้ดขึ้นแล้ว **ปิดไฟล์ Google Sheet แล้วเปิดใหม่** จะเห็นเมนู **🏪 sinthaiPOS**
โผล่ขึ้นมาบนแถบเมนู ข้างๆ "ส่วนขยาย"

| คำสั่ง | ทำอะไร |
|---|---|
| 🔍 **ตรวจสอบข้อมูลสินค้า** | ไล่ตรวจทุกรายการแล้วสร้างแผ่น `ตรวจสอบข้อมูล` บอกว่าอะไรผิดและควรทำอย่างไร เรียงจากเรื่องที่ทำให้เสียเงินก่อน |
| ➕ **เพิ่มสินค้าใหม่** | เพิ่มแถวพร้อมรหัส SKU ที่ไม่ซ้ำของเดิม แล้วบอกว่าต้องกรอกอะไรต่อ |
| ⚙️ **ตรวจความพร้อมระบบ** | เหมือน `checkSetup()` แต่แสดงเป็นกล่องข้อความ ไม่ต้องเปิด Execution log |

**สิ่งที่รายงานจับได้:** ขายต่ำกว่าทุน (พร้อมบอกราคาที่ควรขาย), ไม่มีราคาขาย, สต็อกเป็น 0,
บาร์โค้ดซ้ำ, บาร์โค้ดที่หลักตรวจสอบผิด, รหัส SKU ซ้ำ, ไม่รู้ต้นทุน

> ปัญหาชนิดที่เจอเกิน 20 รายการจะยุบเหลือบรรทัดสรุปบรรทัดเดียว — ข้อมูลจริงของร้าน
> ให้ผล 429 บรรทัด ซึ่งทำให้ 6 รายการที่ขาดทุนจริงจมหายไป พอยุบแล้วเหลือ 22 บรรทัด
> ที่ตัดสินใจได้ทุกบรรทัด
...
ใช้ตัวจำลอง Sheets API ใน `tests/apps-script/harness.mjs` ครอบคลุม 69 เคส:
